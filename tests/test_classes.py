...
def timeslots():
    return [Timeslot(
        ...
        start=datetime(2025, 9, 25, 9, 30),
        end=datetime(2025, 9, 25, 16, 0)
    )]
...
def test_get_overlapping_selections(timeslots):
    ...
    group1 = ScoutGroup(
        ...
        agegroup=12,
        size=10,
        available_timeslots=set(timeslots)
    )
    group2 = ScoutGroup(
        name="Wolves",
        identifier="G2",
        agegroup=13,
        size=8,
        available_timeslots=set([timeslots[1]])
    )
    ...
    # Act
    problem = AssigningActivititesProblem(activities = [activity1, activity2], 
