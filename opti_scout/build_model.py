...
from mip import BINARY, xsum, Model, maximize, OptimizationStatus
import json
import os
    ...
    for g in groups:
        for p in g.priorities:
            ...
            for s in activity.available_sessions:
                x[(g.identifier, activity.identifier, s.startname())] = model.add_var(
...
unavailable = add_age_constraint(
    model, assign_activity_problem.scoutgroups, assign_activity_problem.activities, x, unavailable
)
